...
def format_london_time(iso_timestamp: str) -> str:
    try:
        dt = datetime.datetime.fromisoformat(iso_timestamp)
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=datetime.timezone.utc)
        return dt.astimezone(LONDON_TZ).strftime("%Y-%m-%d %H:%M:%S %Z")
    except Exception:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not self.target_channel:
            error_embed = discord.Embed(
                ...
                description=(
                    "The selected channel no longer exists. Please pick another channel and try again."
                ),
        ...
        try:
            if self.schedule_at is None:
                ...
                success_embed = discord.Embed(
                    ...
                    description=(
                        f"Scheduled for {self.schedule_at.astimezone(LONDON_TZ).strftime('%Y-%m-%d %H:%M:%S %Z')} "
                        f"in {self.target_channel.mention}.\nJob ID: `{job['id']}`"
                    ),
                    color=discord.Color.green(),
                )
            await interaction.followup.edit_message(
    ...
    async def handle_error(self, e, original_response, interaction):
        ...
        if e.status == 403:  # No access to channel
            ...
            error_embed = discord.Embed(
                ...
                description=(
                    f"I cannot post in {self.target_channel.mention}. Give me Send Messages permission there and try again."
                ),
                color=discord.Color.red(),
            )
            await interaction.followup.edit_message(
            ...
            error_embed = discord.Embed(
                title="Channel Missing",
                description=(
                    "That channel could not be found. Choose a valid channel and resend the message."
                ),
                color=discord.Color.red(),
            )
            await interaction.followup.edit_message(
            ...
            error_embed = discord.Embed(
                title="Rate Limited",
                description="Discord asked me to slow down. Please wait a few seconds before trying again.",
                color=discord.Color.red(),
            )
            await interaction.followup.edit_message(
                message_id=original_response.id, content="", embed=error_embed
            ...
            error_embed = discord.Embed(
                title="Discord Issue",
                description=(
                    f"Discord could not deliver the message to {self.target_channel.mention}. Please try again shortly."
                ),
                color=discord.Color.red(),
            )
            await interaction.followup.edit_message(
            ...
            error_embed = discord.Embed(
                ...
                description=(
                    f"I could not send the message in {self.target_channel.mention}. Please check the channel settings and try again."
                ),
                color=discord.Color.red(),
            )
            await interaction.followup.edit_message(
...
class Message(commands.Cog):
    ...
    @app_commands.describe(
        ...
        schedule_at=(
            "Optional schedule date/time. Supports `YYYY-MM-DD HH:MM` (London time) or ISO 8601."
        ),
        ...
        try:
            ...
            if schedule_at:
                try:
                    ...
                    error = make_embed("Invalid Schedule", str(e), discord.Color.red())
                    await interaction.response.send_message(embed=error, ephemeral=True)
                    return
    ...
    @app_commands.command(
        name="scheduled",
        description="List currently scheduled custom messages and embeds for this server.",
    )
    async def scheduled(self, interaction: discord.Interaction):
        dispatcher = get_dispatcher(interaction.client)
        jobs = await dispatcher.list_jobs_for_guild(interaction.guild_id)
        if not jobs:
            await interaction.response.send_message(
                embed=make_embed(
                    "Scheduled Items",
                    "There are no scheduled custom messages or embeds.",
        ...
        for idx, job in enumerate(jobs[:20], start=1):
            payload_type = job.get("payload", {}).get("type", "unknown")
            when = job.get("execute_at", "unknown")
            lines.append(
                f"{idx}. `{job.get('id')}` • **{payload_type}** • <#{job.get('channel_id')}> • `{format_london_time(when)}`"
            )

        if len(jobs) > 20:
            lines.append(f"...and {len(jobs) - 20} more scheduled item(s).")

        await interaction.response.send_message(
            embed=make_embed(
                "Scheduled Items (London Time)",
                "\n".join(lines),
                discord.Color.blurple(),
            ),
            ephemeral=True,
    ...
    @app_commands.command(
        ...
        description="Cancel a scheduled custom message or embed by job ID.",
    )
    @app_commands.describe(job_id="The scheduled job ID shown in /scheduled.")
    async def cancel_scheduled(self, interaction: discord.Interaction, job_id: str):
        dispatcher = get_dispatcher(interaction.client)
        cleaned_job_id = job_id.strip()
        cancelled = await dispatcher.cancel_job(interaction.guild_id, cleaned_job_id)
        if cancelled:
            await interaction.response.send_message(
                embed=make_embed(
                    "Scheduled Item Cancelled",
                    f"Cancelled scheduled item `{cleaned_job_id}`.",
                    discord.Color.green(),
                ),
                ephemeral=True,
            )
        ...
        await interaction.response.send_message(
            embed=make_embed(
                ...
                f"No scheduled item with job ID `{cleaned_job_id}` exists for this server.",
                discord.Color.red(),
            ),
            ephemeral=True,
        )
